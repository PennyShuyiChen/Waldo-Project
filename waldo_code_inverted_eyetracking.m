...
data_folder = strcat('Waldo_Inverted_Data');
...
% conditions and trials
nrepeat = 1; % 50 for the complete experiment 
...
% randomize sequence
nblocks = 1; % 5 for the complete experiment 
...
for seq = eseq
    ...
    Screen('DrawTexture',w,Target,[],scaledRect,180); %rotate the Tface for 180 deg
    Screen('Flip',w);
    ...
    % For inverted faces, use opposite gaze direction before rotation
    helper_inv = zeros(size(helper)); % updated by PSC 
    s = size(helper);
    for inv = 1: s(1)
        for invc = 1:s(2)
            
        helper_inv(inv,invc) = 25-helper(inv,invc)+1;
        end
    end 
    ...
    if whichcoh == 1
        ...
        distractor_gaze = helper_inv(distractor_loc); %invert gaze before rotation
         
    elseif (0 < whichcoh) && (whichcoh < 1)
        kk = (length(distractor_loc)*whichcoh);
        dd = (length(distractor_loc)) - kk;
         
        distractor_gaze = helper_inv(distractor_loc(1:kk)); % invert gaze before rotation
    ...
    for ii = 1:whichndist
        Screen('DrawTexture',w,Distractors(ii),[],coordD(ii,:),180); % rotate the Dfaces for 180 deg
    end
         
    %Draw Target on to the Screen
    Screen('DrawTexture',w, Target_Gaze,[],coordT,180); % rotate the Tface for 180 deg
    Screen('Flip',w);
    ShowCursor('Arrow',w)
...
%     eye_count = 0;
    ...
    for ii = 1:whichndist
        Screen('DrawTexture',w,Distractors(ii),[],coordD(ii,:), 180);
    end
    Screen('DrawTexture',w, Target_Gaze,[],coordT,180);
    ...
    data_cell{seq, saveindex} = RT; %column 4
...
%         movefile(fullfile('.', [edfFile '.edf']), fullfile('.', 'EDF Data', [edfFile '.edf']));
        movefile(fullfile('.', edfFile), fullfile('.', 'Waldo_Inverted_Data', edfFile)); % 'EDF Data' is replaced by 'Waldo_Inverted_Data'
    end
