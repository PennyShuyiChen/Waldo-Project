...
% conditions and trials
nrepeat = 50; % for complete experiment is 50
...
% randomize sequence
nblocks = 5; % for complete experiment is 5
...
%         if Eye_Tracking == 1 && eye_count <= eye_count_length
% %             if Eyelink('NewFloatSampleAvailable') > 0 && eye_count <= eye_count_length
%             evt = Eyelink('NewestFloatSample');
%             eye_count = eye_count+1;
%             eye_data(trial).time_stamp(eye_count) = evt.time;
%             eye_data(trial).eyeX(eye_count,:) = evt.gx;
%             eye_data(trial).eyeY(eye_count,:) = evt.gy;
%             eye_data(trial).pupil(eye_count,:) = evt.pa;
%         end
...
%         movefile(fullfile('.', [edfFile '.edf']), fullfile('.', 'EDF Data', [edfFile '.edf']));
        movefile(fullfile('.', edfFile), fullfile('.', 'Waldo_Data', edfFile)); % 'EDF Data' is replaced by 'Waldo_Inverted_Data'
    end
