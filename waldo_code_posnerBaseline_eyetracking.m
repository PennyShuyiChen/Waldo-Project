...
% Modified by Penny Shuyi Chen 06/26/2019 for Posner Paradigm-proportion
% version 
...
Eye_Tracking    = 1; % 1: we will track the eyes / 0: no eye tracking
...
valid = [0,0,0,0,1,2,2,2,2,2]; %0-invalid | 1-valid | 12-baseline
...
% conditions and trials
nrepeat = 15; % with the baseline is 15
...
[eseq, emat] = randseq(emat, 30);



check = emat(emat(:,1)<31,:);
checkexp = emat(emat(:,1)>30,:);

% randomize sequence
%nblocks = 2; % complete experiment is 2
%ntrialsblock = 100/nblocks %(nrepeat * 10)/nblocks; %65 trials/block
...
%     eye_count = 0;
    ...
    data_cell{seq, saveindex +9} = block;% 14
    data_cell{seq, saveindex +10} = targDistancefromCenter;
    if whichvalid == 0
    data_cell{seq, saveindex +11} = pseudo_target_id;%16
...
%if trial == 30 || trial == 90
    if trial == 30 || trial == 90 % updated by WP
